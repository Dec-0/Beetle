...
require Exporter;
@ISA = qw(Exporter);
@EXPORT = qw(BinListGet BinSearch DayStamp TimeStamp DayTimeStamp ScriptBegin SimpleTimeStamp TimeString Sec2String StorageAddComma RandString IfDirExist IfFileExist IfFileAllExist FileLock FileUnLock ReturnLog SplitAndJoin);
...
sub BinSearch
{
	...
	die "[ Error ] $Cols[1] for $BinName not exist!\n" unless(length($Cols[1]) > 0 && (-e $Cols[1] || $JumpConfirmFlag ));
...
sub SimpleTimeStamp
{
	...
	my $Year = $temp_time[5] + 1900;
	my $Month = $temp_time[4] + 1;
	my $Day = $temp_time[3];
	my $Hour = $temp_time[2];
	my $Min = $temp_time[1];
	my $Sec = $temp_time[0];
	...
	my $SimpleTStamp = join("",$Year,$Month,$Day,$Hour,$Min,$Sec);
...
sub SplitAndJoin
{
	my ($String,$Separator,$IdTo,$IdFrom) = @_;
	$IdFrom = 0 unless($IdFrom);
	
	my @Items = split /$Separator/, $String;
	if($IdTo == 0)
	{
		$IdTo = 0;
	}
	...
	my $NewString = join("$Separator",@Items[$IdFrom .. $IdTo]);
